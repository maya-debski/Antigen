...
html_meta = {
    "antigen-linkify": "enabled" if LINKIFY_ENABLED else "disabled",
}

# Print a concise build-time note (helps when scanning Sphinx logs)
print(
    f"[Antigen docs] MyST linkify backend: {'ENABLED' if LINKIFY_ENABLED else 'DISABLED'}"
)
...
if _requested_theme and _requested_theme in _supported:
    if not _try_set_theme(_supported[_requested_theme]):
        print(
            f"[Antigen docs] Requested theme '{_requested_theme}' not installed; falling back."
        )
